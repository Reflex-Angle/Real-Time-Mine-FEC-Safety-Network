...
public class Application {
	...
	private Map<String, ModuleCallback> moduleCallbacks = new HashMap<>();
	 ...
	 * @param edgeType
	 */
	public void addAppEdge(String source, String destination,double delay,  double tupleCpuLength, 
			double tupleNwLength, String tupleType, int direction, int edgeType){
		AppEdge edge = new AppEdge(source, destination,delay, tupleCpuLength, tupleNwLength, tupleType, direction, edgeType);
		getEdges().add(edge);
		getEdgeMap().put(edge.getTupleType(), edge);
	}
	...
	public void setModuleCallback(String moduleName, ModuleCallback callback) {
        moduleCallbacks.put(moduleName, callback);  // Store the callback in the map
        System.out.println("Callback set for module: " + moduleName);
    }

    // Method to trigger the callback (to be called when needed)
    public void triggerCallback(String moduleName, Object data) {
        ModuleCallback callback = moduleCallbacks.get(moduleName);
        if (callback != null) {
            callback.onCallback(data);  // Execute the callback with the data
        } else {
            System.out.println("No callback found for module: " + moduleName);
        }
    }
